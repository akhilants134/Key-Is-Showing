...
**Before (DevTools):** A request to `api.openai.com` showed `Authorization: Bearer sk-...` in Request Headers (key visible in the browser Network tab).
...
**After (DevTools):** No request to `api.openai.com`; only `POST /api/summarize` to the app backend. The Authorization header is completely absent from all browser network traffic. The only request the browser makes is a `POST /api/summarize` to the app's own backend with a JSON body containing the notes. No LLM credentials appear anywhere in the browser.
